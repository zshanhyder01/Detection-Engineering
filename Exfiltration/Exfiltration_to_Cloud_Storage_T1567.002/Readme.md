...
|---|---|---|---|
| 1 | `rclone_execution` | process_creation | Execution of `rclone` (the most abused cloud-exfil tool), including renamed binaries via flag combinations. |
| 2 | `cloud_cli_upload_tool` | process_creation | Execution of cloud storage CLIs (MEGAcmd/megatools, `aws s3`, `gsutil`, `az storage`) in an upload mode. |
| 3 | `script_cloud_upload` | ps_script | Scripted/manual upload primitives (`Invoke-WebRequest`, `WebClient`, `curl`) targeting cloud storage API endpoints. |
| 4 | `cloud_storage_network_connection` | network_connection | A **non-browser** process making a network connection to a known cloud storage service. |
| 5 | `cloud_storage_large_upload` | proxy | A single large (`≥10 MB`) `PUT`/`POST` upload to a cloud storage host. |
| 6 | `s3_bucket_public_exposure` | aws:cloudtrail | An S3 bucket being made public or shared to an external principal (cloud-native exfil path). |
| 7 | `data_staged_archive` | process_creation | Data being compressed/password-protected into an archive — the staging precursor to exfiltration. |
